...
const PlaceOrderScreen = ({ history }) => {
  ...
  cart.total = (
    ...
    Number(cart.totalShippingPrice) +
    Number(cart.tax)
  ).toFixed(2);
                  ...
                  return (
                    <Paper className={classes.drawerPaper} key={idx}>
                      <ListItem>
                        ...
                        />
                        <ListItemText
                          primary={`${item.quantity} x £${item.price} = £${
          ...
          <Grid item xs={12} md={4} className={classes.main}>
            <Card className={classes.root} variant="outlined" align="center">
              <CardContent justify="space-between">
                <div>
                  <Typography variant="h5" gutterBottom algin="center">
                    Order Summary
                  </Typography>
                </div>
                ...
                <div>{error && <Message status="error" text={error} />}</div>
              </CardContent>
              <CardActions>
                <Button
                  variant="contained"
